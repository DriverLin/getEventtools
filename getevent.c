...
static void print_event(char *device_name, int type, int code, int value, int print_flags, int pipe_fd)
{
    char buffer[64];
        ...
        for (i = 1; i < nfds; i++)
        {
            if (ufds[i].revents)
            {
                if (ufds[i].revents & POLLIN)
                {
                    ...
                    print_event(device_names[i], event.type, event.code, event.value, print_flags, pipe_fd);
